...
X = df[list(df.columns[1:5])+list(df.columns[6:18])+ list(df.columns[18:-3])]
y = df[df.columns[-1]]
Xt = dt[list(dt.columns[1:5])+list(dt.columns[6:18])+ list(dt.columns[18:-3])]
# Convertir las columnas categóricas a variables numéricas
X_encoded = pd.get_dummies(X)
X_test = pd.get_dummies(Xt)

#X_train, X_betatest, y_train, y_betatest = train_test_split(X_encoded, y, test_size=0.15)
